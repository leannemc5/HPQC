...
```
mpicc my_string_wave_parallel.c -o bin/string_wave_mpi
...
![Baseline Wave with 100 points](output_parallel.gif)
![Increased points from 100 to 1000](output_parallel1.gif)
![Increased points from 1000 to 10,000](output_parallel2.gif)
...
### Activity 3:
The simulations produced 4 separate gifs representing the implementation of rk4 method, rk4 with a small spring interaction, rk4 spring mass and damping system, and finally rk4 spring mass damping system with an increased amount of cycles.
![rk4 method](animate_string_file1.gif)

In all cases, it is clearly seen there is motion in the 1st point of the string. 
It has a strong oscillatory behavior moving up and down with a large amplitude.In the rk4 model, the motion was only limited to the 1st point with no visible motion seen along the rest of the strong, when the spring mass system was added, a small amount of propagation became with nodes very close to the driven node. 
In these nodes they had a small oscillation, proving that the forces between neighbouring points were now being applied. The amplitude of the motion was much less than the driven point, and the wave didn’t travel along the string.
![rk4 & mass spring](animate_string_file2.gif)

When damping was added, the oscillations reduced in magnitude. The motion of the first point remained visible but slightly softened, while the already small oscillations in the neighbouring nodes became even less pronounced.
Increasing the number of cycles extended the duration of the simulation, allowing the small oscillations to be seen across a greater number of points, however the same limited motion remained.
![rk4&mass spring& damping](animate_string_file3.gif)
...
### Activity 3:
The results showed that each stage of the freeform model produced the expected physical behaviour, but also highlighted the limitations in how effectively motion propagates through the system.
The addition of rk4 successfully introduced a more accurate numerical method for updating both position and velocity. Unlike the original model, which directly shifted values between points, the rk4 method integrates the motion over time using intermediate steps.
To make the system more realistic, velocity was introduced as a key variable. Instead of directly updating the position of the points, the system now updated from force to acceleration, velocity and finally position.
In the rk4 method, no interaction points were included, so the energy introduced at the first point couldn’t propagate, which explains why only the 1st point could oscillate. 
To fix this, the spring mass was added. By using Hooke’s law the motions were able to transfer between points, which is why the small oscillations appeared in points close to the driver. However the limited propagation suggests  that the spring constant or timestep might not be enough to transfer the energy properly along the string.
The effect of adding damping to the system removes energy from the system which results in a reduced amplitude for each oscillation, especially in the already weakly oscillating points. While this made the model more realistic, it suppressed the propagation further especially when the initial energy transfer is already small.
While the original model produced visually smooth waves, the updated model gives a more disjointed appearance. This is likely due to the the system behaving more like connected masses rather than a continuous medium and the energy not being efficiently transferred along the string.
Overall, the modifications successfully modeled a more realistic simulation, however, these changes also showed that realistic wave propagation strongly depends on how effectively energy is transferred between points.
Further improvements, such as increasing the spring constant, reducing damping, or refining the timestep, would likely enhance wave propagation across the string.
