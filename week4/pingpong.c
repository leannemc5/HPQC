// needed libraries
#include <stdio.h>
#include <stdlib.h>
#include <mpi.h>

int check_args(int argc, char **argv);
void run_pingpong(int my_rank, int num_pings);

int main(int argc, char **argv)
{
    int ierror = 0;
    int my_rank, uni_size;

    // check arguments before MPI Init
    int num_pings = check_args(argc, argv);
    // initialsie mpi enviroment
    MPI_Init(&argc, &argv);
    // get rank and total numbere of process
    MPI_Comm_rank(MPI_COMM_WORLD, &my_rank);
    MPI_Comm_size(MPI_COMM_WORLD, &uni_size);

    // ensure only 2 processes are used
    if (uni_size != 2)
    {
        if (my_rank == 0) 
            fprintf(stderr, "Error: Ping-pong requires exactly 2 processes.\n");
        MPI_Finalize(); // clean up mpi enviroment
        return -1;
    }

    // run the benchmark
    run_pingpong(my_rank, num_pings);

    MPI_Finalize();
    return 0;
}

void run_pingpong(int my_rank, int num_pings)
{
    int counter = 0; // counter passed between processes
    int tag = 0;  // message tag to match send/receives
    int count = 1; // number of elements to send
    double start_time, end_time;
    MPI_Status status;

    // synchronize processes before starting the timer
    MPI_Barrier(MPI_COMM_WORLD);
    // record start time
    start_time = MPI_Wtime();
    // loop until counter reaches desired numbr of pings
    while (counter < num_pings)
    {
        if (my_rank == 0)
        {
            // Root sends (ping) to process
            MPI_Send(&counter, count, MPI_INT, 1, tag, MPI_COMM_WORLD);
            // Root receives (pong)
            MPI_Recv(&counter, count, MPI_INT, 1, tag, MPI_COMM_WORLD, &status);
        }
        else if (my_rank == 1)
        {
            // client receives (ping)
            MPI_Recv(&counter, count, MPI_INT, 0, tag, MPI_COMM_WORLD, &status);
            // client increments
            counter++;
            // client sends (pong)
            MPI_Send(&counter, count, MPI_INT, 0, tag, MPI_COMM_WORLD);
        }
    }
    // record end time
    end_time = MPI_Wtime();

    // output results from Root
    if (my_rank == 0)
    {
        double elapsed_time = end_time - start_time;
        // Average time per "trip" (one ping + one pong)
        double avg_time = elapsed_time / num_pings;

        printf("Final Counter: %d\n", counter);
        printf("Total Elapsed Time: %f s\n", elapsed_time);
        printf("Average Time per Round-Trip: %f s\n", avg_time);
    }
}
// validate coomand line arguments
int check_args(int argc, char **argv)
{
    //expects only 1 argument, 1 ping
    if (argc != 2)
    {
        fprintf(stderr, "Usage: mpirun -np 2 pingpong [NUM_PINGS]\n");
        exit(-1);
    }
    // convert argument to integer and return it
    return atoi(argv[1]);
}
